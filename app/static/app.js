...
function reset(){if(timer)clearTimeout(timer);timer=null;hide();idleCopy.hidden=false;urlInput.value="";progressBar.style.width="0%";progressValue.textContent="0%"}
function manual(msg=""){hide();idleCopy.hidden=true;manualForm.hidden=false;urlInput.placeholder=msg||"https://...";urlInput.focus()}
function error(msg){hide();idleCopy.hidden=true;errorPanel.hidden=false;$("errorText").textContent=msg||"تعذر تجهيز الفيديو."}
...
function ready(j){hide();idleCopy.hidden=true;readyPanel.hidden=false;readyTitle.textContent=j.title||"الفيديو جاهز";readyPlatform.textContent=(j.platform||"Video")+" · الملف جاهز للتحميل";downloadButton.href=j.download_url}
async function api(path,opt={}){const r=await fetch(path,{...opt,headers:{"Content-Type":"application/json",...(opt.headers||{})}});let d={};try{d=await r.json()}catch{}if(!r.ok)throw new Error(d.error||"حدث خطأ أثناء الاتصال بالخادم.");return d}
function extract(v){const m=String(v||"").trim().match(/https?:\/\/[^\s<>"']+/i);return m?m[0].replace(/[),.;!?،؛]+$/u,""):""}
async function create(v){const u=extract(v);if(!u)return manual("ألصق رابطًا كاملًا يبدأ بـ https://");progress({status:"queued",progress:1,platform:"VIDEO"});try{const j=await api("/api/jobs",{method:"POST",body:JSON.stringify({url:u})});progress(j);poll(j.id)}catch(e){error(e.message)}}
async function poll(id){try{const j=await api("/api/jobs/"+id);if(j.status==="ready"||j.status==="served")return ready(j);if(j.status==="error")return error(j.error);progress(j);timer=setTimeout(()=>poll(id),900)}catch(e){error(e.message)}}
